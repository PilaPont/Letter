...
class TestAccessRights(TestLetter):
    ...
    #     print('start see all')
    #     self.letter_out.sudo(self.user_see_all).read()
    #     print('ended see all')

    # def test_follower(self):
    #     print('start follower')
    #     self.letter_out.sudo(self.user_follower).read()
    #     self.letter_out.sudo(self.user_follower).write({
    #         'subject': 'test 11111',
    #     })
    ...
    def test_user_config(self):
        ...
        with self.assertRaises(AccessError):
            self.content.sudo(self.user_signa).read()

        self.content.sudo(self.user_config).read()
        self.header.sudo(self.user_config).read()

        self.content.sudo(self.user_config).write({
            'name': 'testk',
        })

        self.header.sudo(self.user_config).write({
            'name': 'temp two',
            'language_id': 1,
        })
        with self.assertRaises(AccessError):
            self.header.sudo(self.user_signa).write({
                'name': 'temp two',
            })

        self.env['letter.content_type'].sudo(self.user_config).create({
            'name': 'test3',
        })
        with self.assertRaises(AccessError):
            self.env['letter.content_type'].sudo(self.user_see_all).create({
                'name': 'test3',
            })

        self.env['letter.header'].sudo(self.user_config).create({
            'name': 'header aria',
            'language_id': 1,
        })
        with self.assertRaises(AccessError):
            self.env['letter.header'].sudo(self.user_see_all).create({
                'name': 'header aria',
                'language_id': 1,
            })

        with self.assertRaises(AccessError):
            self.header.sudo(self.user_see_all).unlink()

        with self.assertRaises(AccessError):
            self.content.sudo(self.user_signa).unlink()

        self.content.sudo(self.user_config).unlink()
        self.header.sudo(self.user_config).unlink()
